...
- [ ] What problem does the context API help solve?
  - It solves complex state management by allowing you to access state globally via useContext and createContext
- [ ] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
  - Reducers are the initial state, as well as the cases in which state is to be set
  - Actions are type of modification we want to perform on the state, as well as the action we will take
  - The store is where the state is managed, and in order to access the global state and actions via redux you must connect your app to the store
- [ ] What is the difference between Application state and Component state? When would be a good time to use one over the other?
  - Application state is application-wide and can be accessed anywhere via connect or context, whereass Component state is local to that component until it is passed on. Form inputs are a good use case for component state
- [ ] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?
  - Redux thunk is middleware that lets us to use asynchronous actions in the otherwise synchronous redux.
- [ ] What is your favorite state management system you've learned and this sprint? Please explain why!
  - I like redux, even though there is a lot of boilerplate it has a lot of potential to make scaleable apps easier to reason about
